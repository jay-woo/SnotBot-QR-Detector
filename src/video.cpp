...
int main ( int argc, char **argv )
{
	...
	while(key != 'q')				// While loop to query for Image Input frame
	{
		...
		if (mark >= 2)		// Ensure we have (atleast 3; namely A,B,C) 'Alignment Markers' discovered
		{
			...
			midpoint = (mc[A] + mc[B]) * 0.5;
			
			qr_msg.x = midpoint.x;
			qr_msg.y = midpoint.y;
			qr_msg.z = sqrt( pow(mc[A].x - mc[B].x, 2) + pow(mc[A].y - mc[B].y, 2) );
			qr_msg.w = slope_rad;
			...
			if( top < contours.size() && right < contours.size() && bottom < contours.size() && contourArea(contours[top]) > 10 && contourArea(contours[right]) > 10 && contourArea(contours[bottom]) > 10 )
			{
				...
				if(DBG==1)
				{
					...
					putText(traces, "POSITION:", Point(20,30), fontFace, 0.5, Scalar(255,255,255), 1, 8);
					putText(traces, "SLOPE:", Point(20,50), fontFace, 0.5, Scalar(255,255,255), 1, 8);
					putText(traces, "SIZE:", Point(20,70), fontFace, 0.5, Scalar(255,255,255), 1, 8);

					stringstream ss;
					ss << "(" << qr_msg.x << ", " << qr_msg.y << ")";
					string position_text = ss.str();

					ss.str(string());
					ss << qr_msg.w;
					string slope_text = ss.str();

					ss.str(string());
					ss << qr_msg.z;
					string size_text = ss.str();

					putText(traces, position_text, Point(110, 30), fontFace, 0.5, Scalar(0,0,255), 1, 8);
					putText(traces, slope_text, Point(110, 50), fontFace, 0.5, Scalar(0,0,255), 1, 8);
